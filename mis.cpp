...
void findMIS(const T& graph, std::vector<int>& is, EdgeExtractor edge_extractor)
{
    ...
    for(int node = 0, sort_strategy = 0; node < graph_size; ++sort_strategy){
        ...
        while( !que.empty() ){
            if(sort_strategy == 0) {
                sort(que.begin(), que.end(), [&](const int lhs, const int rhs) -> bool {
        ...
        if( sort_strategy == 1 ) { ++node, sort_strategy = -1; }
    }
}
